...
**Entry:** At market when setup conditions are met.

**Stop loss (wide, structural — let trades breathe):**
At 5x leverage, liquidation is ~18% from entry:
...
- After TP1 is hit: move stop to entry price (breakeven).

**R:R minimum:** R:R must be >= 1.2:1 for TP1.
...
**When to hold:** Position profitable but hasn't reached TP1, or slightly negative but above stop.

**When to take profit:** Price reached TP1 → close 50%, move stop to entry. Price reached TP2 → close all.

**When to close at a loss:** Price hit stop level. Liquidation within 8%. New opposite signal with valid setup.
